...
int Midas::useFarROI(bool far_roi)
{
    if(!_model_buf) return 1;
    _far_roi = far_roi;
    return 0;
...
int Midas::postProcess(int w, int h, std::vector<float> &dep_thres)
{
    ...
    for(cv::Point2f point2F : _pre_points){
        ...
        float y;
        if(_far_roi) {
            x=(point2F.x-_roi.x)/w*TARGET_W_H;
            y=(point2F.y-_roi.y)/h*TARGET_W_H;
        } else {
            x=(point2F.x)/w*TARGET_W_H;
            y=(point2F.y)/h*TARGET_W_H;
        }
...
int Midas::invokeProcessAsync(const cv::Mat &bgr) {
    if(!_model_buf) return 1;
...
void Midas::preProcessImage(const cv::Mat &rgb)
{
    ...
    if(_far_roi) {
        ...
        cv::resize(rgb, in, cv::Size(TARGET_W_H,TARGET_W_H));
    }
